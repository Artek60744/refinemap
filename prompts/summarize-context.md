...
3. `product_memory` is inherited context, not a result of this session: never copy
   its items into `facts`. Include a memorized fact only if THIS session confirmed it
   or made it more precise — and then write the precised version, not the original.
4. `unknowns` must be NEW uncertainties revealed by the answers — contradictions between two
...
5. Add `dependencies` and `risks` when clearly supported by the context.
6. Set `confidence` (`low|medium|high`) based on how well the subject is understood.
7. Set `enoughContext=true` as soon as the DECISION-CRITICAL unknowns are lifted, even if
...
8. The `reason` must briefly justify the decision (for logs and UI) and name which unknown
