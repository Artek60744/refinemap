...
La mémoire produit est le deuxième fossé défensif de RefineMap (après le moteur de raffinement) : les faits durables établis lors d’une session sont injectés dans les sessions ultérieures concernant le **même produit**, de sorte que le moteur ne pose jamais une question dont il connaît déjà la réponse. Ce n’est délibérément pas un journal : seul un ensemble borné de déclarations durables et catégorisées est conservé et réinjecté. Les décisions de conception (pourquoi un ensemble borné de faits plutôt qu’un journal) sont explicitées dans cette page, et la règle de durabilité est énoncée pour le moteur réel dans `prompts/extract-product-memory.md`.
...
- **Validation :** `python -m pytest tests/test_product_memory.py tests/test_product_memory_api.py tests/test_product_memory_flow.py -q`.sts/test_product_memory_api.py tests/test_product_memory_flow.py -q`.